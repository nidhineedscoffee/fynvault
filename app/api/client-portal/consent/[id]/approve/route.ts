...
import { ConsentApprovalSchema, approveConsent } from "@/lib/consent";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const limit = enforceRateLimit({ scope: "consent_approve", key: getClientIp(request), limit: 30, windowMs: 60_000 });
  if (!limit.allowed) {
  ...
  const parsed = ConsentApprovalSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ ok: false, error: "Invalid request payload.", issues: parsed.error.issues }, { status: 400 });
  }

  const { id } = await context.params;
  return serviceResponse(await approveConsent(id, parsed.data.approvedBy));
}
