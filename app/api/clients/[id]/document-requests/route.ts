...
import { DocumentRequestSchema, createDocumentRequest, listRowsForClient } from "@/lib/mvp";
...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  return serviceResponse(await listRowsForClient("document_requests", id));
}
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ ok: false, error: "Invalid document request payload.", issues: parsed.error.issues }, { status: 400 });
  }
  ...
  return serviceResponse(await createDocumentRequest(id, { ...parsed.data, firmId: access.firmId }));
}
